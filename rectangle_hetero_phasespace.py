...
# === PARAMETERS ===
# Simulation setup: heterogeneous swarm in rectangular box with variable curvity means
params = 8
dt = 1E-5
T = 1E5
tSave = 10000
...
def compute_rectangular_coverage(df, boxSize, cell_size=50):
    """
    Compute area coverage by checking which grid cells in a rectangular box
...
# === FULL SIMULATION: HETEROGENEOUS RECTANGULAR PHASE SPACE ===
# For each target mean curvity from -50 to 50, run "repeats" stochastic trials
output_dir = "hetero_rectangular_mean_sweep"
os.makedirs(output_dir, exist_ok=True)

for target_mean in range(-50, 51):  
    ...
    for run_id in range(repeats):
        ...
        cov = compute_rectangular_coverage(df, boxSize)
